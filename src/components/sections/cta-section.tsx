...
export default function CtaSection() {
  ...
  useGSAP(
    () => {
      const gradient = scopeRef.current?.querySelectorAll<HTMLElement>("[data-cta-gradient]");
      const accent = scopeRef.current?.querySelector<HTMLElement>("[data-cta-accent]");
      const globe = scopeRef.current?.querySelector<HTMLElement>("[data-cta-globe]");
      const cards = scopeRef.current?.querySelector<HTMLElement>("[data-cta-cards]");
      const topWrap = scopeRef.current?.querySelector<HTMLElement>("[data-cta-top]");
      const storeLinks = scopeRef.current?.querySelectorAll<HTMLElement>("[data-cta-store]");
      const proofWrap = scopeRef.current?.querySelector<HTMLElement>("[data-cta-proof-wrap]");
      const proofItems = scopeRef.current?.querySelectorAll<HTMLElement>("[data-cta-proof]");
      const visualWrap = scopeRef.current?.querySelector<HTMLElement>("[data-cta-visual-wrap]");

      if (!gradient?.length || !globe || !accent || !cards || !topWrap || !storeLinks?.length || !proofWrap || !proofItems?.length || !visualWrap) return;

      gsap.set(gradient, { scale: 1.16, y: -36, filter: "blur(16px)", transformOrigin: "center center" });
      gsap.set(globe, { y: 36, scale: 1.18, autoAlpha: 0 });
      gsap.set(accent, { scale: 1.5 });
      gsap.set(cards, { y: 128, scale: 0.75, autoAlpha: 0 });
      gsap.set(topWrap, { y: 24, autoAlpha: 0 });
      gsap.set(storeLinks, { scale: 1.2, autoAlpha: 0, transformOrigin: "center center" });
      gsap.set(proofItems, { y: 20, autoAlpha: 0 });
  ...
  return (
    <section ref={scopeRef} className="relative flex flex-col justify-between bg-background h-225 px-5 lg:px-17.5 overflow-hidden">
      ...
      <div className="relative h-full mx-auto bg-background w-full max-w-325 px-4 lg:px-16 xl:px-17.5  py-16">
        <Image src="/grid.svg" alt="Grid Background" fill />
        <div data-cta-top className="relative text-center flex flex-col items-center gap-6">
          ...
          <div className="flex items-center gap-2.5 mb-3">
            <Link data-cta-store className="hover:scale-105 transition-all duration-300 ease-in-out active:scale-90" href="https://play.google.com/" target="_blank">
              <PlayStore />
            </Link>
            <Link data-cta-store className="hover:scale-105 transition-all duration-300 ease-in-out active:scale-90" href="https://apps.apple.com/" target="_blank">
              <AppStore />
            </Link>
          </div>

          <div data-cta-proof-wrap className="flex max-md:flex-col md:w-full md:items-center md:justify-center gap-5">
            <div data-cta-proof className="flex gap-2.5 shrink-0">
      ...
      <div data-cta-visual-wrap className="absolute z-2 pointer-events-none inset-0 flex grow justify-stretch items-end">
        <div className="relative w-full h-full flex flex-col justify-end items-center">
          <Image data-cta-gradient src="/hero-gradient.png" alt="Hero Gradient" width={2000} height={100} className="absolute max-md:hidden h-1/2 object-cover object-top min-w-500 mx-auto" />
          <Image data-cta-gradient src="/hero-gradient-alt.png" alt="Hero Gradient" width={768} height={100} className="absolute md:hidden h-1/2 object-cover object-top min-w-3xl mx-auto" />
          <Image data-cta-accent src="/hero-accent.svg" alt="Hero Accent" width={1440} height={100} className="absolute h-1/2 object-cover object-top mx-auto w-full" />
          <Image data-cta-globe src="/globe.png" alt="Globe" width={760} height={440} className="absolute bottom-0" />
          <Image data-cta-cards src="/cards.svg" alt="Cards" width={1162} height={328} className="absolute min-h-80 object-cover" />
        </div>
      </div>
    </section>
  );
